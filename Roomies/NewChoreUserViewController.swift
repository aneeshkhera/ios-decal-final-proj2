...
class NewChoreForUserViewController: UIViewController, UITextViewDelegate, UITextFieldDelegate, SSRadioButtonControllerDelegate {
    ...
    @IBAction func saveChore(_ sender: AnyObject) {
        ...
        if let loadedData = UserDefaults().data(forKey: thisUser.name) {
        ...
        thisUser.score = String(choreDifficulty+Int(thisUser.score)!)
        ...
        UserDefaults.standard.set(choreData, forKey: thisUser.name)
